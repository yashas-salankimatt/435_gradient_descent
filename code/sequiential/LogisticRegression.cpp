...
LogisticRegression::LogisticRegression(
    ...
    float learning_rate) : _features(features), _labels(labels), _learning_rate(learning_rate),
                           _n_samples(features.size()), _n_features(features[0].size()),
                           _weight(std::vector<std::vector<float>>(features[0].size(), std::vector<float>(1)))
{

    // initialize _weight vector, where -1 <= w_i <= 1
    for (int i = 0; i < _n_features; i++)
    {
        _weight[i][0] = static_cast<float>(rand()) / static_cast<float>(RAND_MAX) - 0.5;
    }
}
